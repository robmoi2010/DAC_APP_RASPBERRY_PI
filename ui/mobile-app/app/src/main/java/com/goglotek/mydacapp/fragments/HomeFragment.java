...
public class HomeFragment extends Fragment {
  ...
  public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle bundle) {
    ...
    homeDataProcessor = GenericDataProcessor.getInstance(
        App.webClientMap.get(WebClientType.HOME_DATA));
    volumeUpProcessor = GenericDataProcessor.getInstance(
        App.webClientMap.get(WebClientType.VOLUME_UP));
    volumeDownProcessor = GenericDataProcessor.getInstance(
        App.webClientMap.get(WebClientType.VOLUME_DOWN));
        ...
        requireActivity().runOnUiThread(() -> {
          ...
          for (Response rsp : finalRespLocal) {
            if (rsp.getKey().equals(Config.getConfig("CURRENT_VOLUME_ID"))) {
              ...
              sb.append(" " + rsp.getDisplayName().trim()).append(":").append(rsp.getValue().trim())
                  .append("\n");
            }
          }
          volumeView =
              volumeView == null ? requireActivity().findViewById(R.id.speedView) : volumeView;
  ...
  private String processHomeText(String currentText,
      String serverText) {//updates only fields received from server and keeps current fields not in server payload
  ...
  private void handleVolumeSliderOnchange(int newVal) {
    ...
    if (newVal != previousSliderVolume) {
      ...
      executor.execute(() -> {
        try {
          GenericDataProcessor processor = GenericDataProcessor.getInstance(
              App.webClientMap.get(WebClientType.VOLUME_UPDATE));
  ...
  private void createHomeDataWebSocketListener() {
    WebSocketClient client = new WebSocketClient(Config.getConfig("BASE_URL") + "system/ws",
