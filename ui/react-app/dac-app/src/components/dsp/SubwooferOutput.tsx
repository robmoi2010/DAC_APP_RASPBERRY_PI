...
import { getSubwooferOutputOptions, updateSubwooferOutput } from "../../services/DspService";

const SubwooferOutput = () => {
    ...
    //initial data load
    useEffect(() => {
        loadDynamicData(getSubwooferOutputOptions(), dispatch, "/Output");
    }, []);
...
const generateComponents = (data: responseDataType[], index: number, navigate: NavigateFunction, dispatch: Dispatch) => {
    ...
    components.push(<Header text="Select Subwoofer Output" />);
    ...
    components.push(<DataRow selected={false} onClick={() => navigate("/Output")} text="Back" type={2} active={index == data.length} />);
    return components;
}
const dataSelection = (selection: number, dispatch: Dispatch) => {
    updateSubwooferOutput(JSON.stringify({ key: "" + selection, value: "" + selection })).then(
        data => {
...
export default SubwooferOutput;