...
import { getVolumeDevice, updateVolumeDevice } from "../../services/SystemService";
...
const VolumeDevice = () => {
    ...
    useEffect(() => {
        ...
        loadDynamicData(getVolumeDevice(), dispatch, "/SystemSettings");
    }, []);
...
const generateComponents = (data: responseDataType[], index: number, navigate: NavigateFunction, dispatch: Dispatch) => {
    ...
    components.push(<Header text="Volume Device" />);
...
const dataSelection = (selection: number, dispatch: Dispatch) => {
    updateVolumeDevice(JSON.stringify({ key: "" + selection, value: "" + selection })).then(
        data => {
