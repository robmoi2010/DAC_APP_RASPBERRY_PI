...
class SubwooferOutputRouting(tk.Frame):
    ...
    def route_on_click(frame, selection, type):
        ...
        if answer:
            router.update_subwoofer_output_source(type)
            if type == 0:
    ...
    def __init__(self, parent, controller):
        ...
        tk.Label(self, text="Select Subwoofer Output Source", font=("Arial", 16)).pack(
            pady=20
        )
        current_output_source = router.get_subwoofer_output_source()
        self.btn1 = tk.Button(
            self,
            relief=RELIEF,
            font=btnFont,
