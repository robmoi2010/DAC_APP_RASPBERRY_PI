...
class ThirdOrderCompensation(tk.Frame):
    ...
    def enable_disable(self, selected):
        ...
        if answer:
            dac.enable_disable_third_order_compensation(selected)
            if selected == 0:
    ...
    def __init__(self, parent, controller):
        ...
        self.row_index=1
        tk.Label(self, text="3rd Order Compensation", font=("Arial", 16)).grid(row=self.get_current_row(), column=0, sticky="nsew")
        active = dac.is_third_order_compensation_enabled()

        self.btn1 = GeneralButton(
            self,
            "Enable",
            selected=active,
            command=lambda: self.enable_disable(0),
        )
